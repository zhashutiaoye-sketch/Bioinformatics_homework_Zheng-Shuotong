...
| Restate the hypothesis in an explicitly **predictive** form (`H_pred`: UC shows a within-patient community change, IBS does not; no *baseline* feature predicts response) and name the rejected competing hypothesis (`H0`: baseline composition separates responders) | `H0` was tested and rejected by the same parameters: PERMANOVA R² = 0.017/0.021, p = 0.868/0.978; alpha BH ≥ 0.61; 0/136 taxa at FDR < 0.05 — the numbers already existed in `tables/`, so nothing new had to be trusted | kept, and the report now states both directions of the same claim (report §6) |
